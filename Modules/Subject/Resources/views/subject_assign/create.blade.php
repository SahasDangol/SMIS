...
@section('title', get_school_info('site_title').' | Optional Subject Assignment | Create')
@section('Body')
    <div class="col-md-12">
        <div class="card ">
            ...
            <div class="card-body">
                ...
                @endphp
                @if(count($students))
                    <form id="TypeValidation" class="form-horizontal process" action="{{route('subject_assign.store')}}"
                          method="post">
                            ...
                            <table class="table table-striped table-no-bordered table-hover" cellspacing="0"
                                ...
                                @foreach($students as $student)
                                    <tr>
                                        ...
                                        <th>
                                            @foreach($optional_subjects_1 as $optional_subject)
                                                {{--{{$student->optional_subjects}}--}}
                                                <div class="form-check">
                                                    <label class="form-check-label">
                                                        <input class="form-check-input" type="radio"
                                                               name="radio_1_{{$student->id}}"
                                                               value="{{$optional_subject->id}}"
                                                               @if($student->optional_subjects)
                                                               @if($optional_subject->id==$student->optional_subjects->optional_subject1_id)
                                                               checked=""
                                                                @endif
                                                                @endif
                                                        >
                                                        {{ucfirst($optional_subject->subject_name)}}
                                                        <span class="circle">
                                        ...
                                        <th>
                                            @foreach($optional_subjects_2 as $optional_subject)
                                                <div class="form-check">
                                                    <label class="form-check-label">
                                                        <input class="form-check-input" type="radio"
                                                               name="radio_2_{{$student->id}}"
                                                               value="{{$optional_subject->id}}"
                                                               @if($student->optional_subjects)
                                                               @if($optional_subject->id==$student->optional_subjects->optional_subject2_id)
                                                               checked=""
                                                                @endif
                                                                @endif
                                                        >
                ...
                @else
                    <div>
                        <br>
                        <br><h4 class="text-center">Students Not Found!!</h4></div>
                @endif
            </div>
        </div>
    </div>
